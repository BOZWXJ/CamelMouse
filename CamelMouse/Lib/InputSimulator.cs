...
namespace MMFrame.Windows.Simulation
{
	...
	public static class InputSimulator
	{
		...
		{
			AddMouseInput(ref inputs, new List<MouseStroke> { flag }, data, absolute, x, y);
		}
		...
		/// <param name="x">水平位置または移動量</param>
		/// <param name="y">垂直位置または移動量</param>
		public static void AddMouseInput(ref List<Input> inputs, List<MouseStroke> flags, int data, bool absolute, int x, int y)
		{
			if (flags == null) {
				return;
			}
		...
		public static void AddMouseInput(ref List<Input> inputs, int flags, int data, int x, int y, int time, int extraInfo)
		{
		...
		public static void AddKeyboardInput(ref List<Input> inputs, KeyboardStroke flags, Keys key)
		{
		...
		public static void AddKeyboardInput(ref List<Input> inputs, int flags, short virtualKey, short scanCode, int time, int extraInfo)
		{
			Input input = new Input();
